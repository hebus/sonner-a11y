...
let versionCommit = '';
try {
    versionCommit = capture(`git log -1 --format=%H -S"\\"version\\": \\"${version}\\"" -- package.json`);
} catch {
...
if (!versionCommit) {
    ...
    const drifted = capture(`git diff --name-only ${versionCommit}..HEAD -- ${SOURCE_PATHS.join(' ')}`);
    if (drifted) {
        console.error('\n  Sources changed after the version was set:\n');
        drifted.split('\n').forEach(f => console.error(`    ${f}`));
        fail(`npm, the changelog and git would describe different trees. Add a changeset and let Changesets bump the version.`);
    }
...
for (const entry of files) {
    ...
    if (!existsSync(path)) fail(`"${entry}" is declared in files but does not exist. The tarball would be missing it.`);
...
if (!assumeYes) {
    ...
    const answer = (await rl.question(`\n  Publish ${tag} to npmjs and push the tag? [y/N] `)).trim().toLowerCase();
    rl.close();
