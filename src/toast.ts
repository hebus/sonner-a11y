...
export function addToast(options: ToastType) {
  ...
  if (options.type) {
    ...
    if (options.type === "loading" && oldToast) {
      const currentTime = oldToast
        .querySelector("[data-icon] div")
  ...
  if (duration > 0) {
    ...
    toast.addEventListener("mousedown", (downEvent) => {
  ...
  announce(
    buildAnnounceText(content, options.action?.label),
    resolvePoliteness(options.type, important),
  );
