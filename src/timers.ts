...
type TimerType = { timeoutId: number | null; remainingTime: number; startTime: number; onExpire: () => void };
...
export function startTimer(id: ToastId, duration: number, onExpire: () => void) {
    ...
    const timer: TimerType = { timeoutId: null, remainingTime: duration, startTime: Date.now(), onExpire };
    timers.set(id, timer);
