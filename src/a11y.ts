  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      if (
        ...
        target.closest("[data-sonner-toasters]")
      )
        return;
...
function isFocusable(toast: HTMLElement) {
  ...
  return (
    state !== "deleting" &&
    state !== "invisible" &&
    !toast.hasAttribute("inert") &&
    toast.getAttribute("aria-hidden") !== "true"
  );
}
...
function focusTarget(region: HTMLElement): HTMLElement | null {
  const preferred =
    lastToasterPosition && region.querySelector(`ol[data-position="${lastToasterPosition}"]`);
  const toasters = preferred
    ? [preferred]
    : [...region.querySelectorAll("[data-sonner-toaster]")].reverse();
 ...
 */
export function setExpandSource(
  toaster: HTMLElement | null,
  source: ExpandSourceType,
  active: boolean,
) {
  if (!toaster) return;
...
function collapseAll(region: HTMLElement) {
  region
    ...
    .forEach((toaster) => setExpandSource(toaster, "focus", false));
}
...
function attachHotkey(region: HTMLElement) {
  ...
  document.addEventListener("keydown", (event) => {
    ...
    const pressed = config.a11y.hotkey.every(
      (key) => (event as unknown as Record<string, boolean>)[key] || event.code === key,
    );
    if (!pressed) return;
